...
user_skill_dict = dblp.get_user_skill_dict(
    dblp.load_preprocessed_dataset(file_path='dataset/dblp_preprocessed_dataset_V2.3.pkl'))
foldIDsampleID_strata_dict = dblp.get_foldIDsampleID_stata_dict(
    data=dblp.load_preprocessed_dataset(file_path='dataset/dblp_preprocessed_dataset_V2.3.pkl'),
    train_test_indices=dblp.load_train_test_indices(file_path='dataset/Train_Test_Indices_V2.3.pkl'),
    kfold=10)
preprocessed_authorNameId_dict = dblp.get_preprocessed_authorNameID_dict(file_path='dataset/authorNameId.txt')
graph_handler = DBLPGraph()
graph_handler.load_files()
#graph_handler.write_graph()
graph_handler.read_graph()


model = 'ablation/predictions/DBLP_512_45k.csv'
...
for file_name in file_names:
    ...
    k_max = 20
    #print(f"kfold is {k_fold}")
    #print(f"true indices are {true_indices}")
    # eval settings
    ...
    # writing output file
    result_output_name = "output/eval_results/Communication_{}.csv".format(method_name)
    with open(result_output_name, 'w') as file:
        ...
        for i in fold_set:
            ...
            for j in evaluation_k_set:
                ...
                #print(f"pred/true ind are",pred, truth)
